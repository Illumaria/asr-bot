...
def normalize_batch(
    x: torch.Tensor, seq_len: torch.Tensor, normalize_type: Union[str, Dict[str, Any]]
    ...
    if normalize_type == "per_feature":
        ...
        for i in range(x.shape[0]):
            if x[i, :, : seq_len[i]].shape[1] == 1:
                raise ValueError(
                    "normalize_batch with `per_feature` normalize_type received "
                    "a tensor of length 1. This will result in torch.std() "
                    "returning nan. Make sure your audio length has enough samples "
...
class FilterbankFeatures(nn.Module):
    ...
    def __init__(
        ...
        if exact_pad and n_window_stride % 2 == 1:
            raise NotImplementedError(
                ...
                f"If audio_length % hop_size == 0. Then the returned spectrogram "
                f"would not be of length audio_length // hop_size. "
                f"Please use an even hop_size."
            )
        ...
        # log_zero_guard_value is the small value we want to use,
        # we support an actual number, or "tiny", or "eps"
        self.log_zero_guard_type = log_zero_guard_type

    def log_zero_guard_value_fn(self, x) -> Union[float, str]:
    ...
    def forward(self, x: torch.Tensor, seq_len: torch.Tensor):
        """
        Get MFCCs for given raw audio signal
        :param x: tensor of shape [B, T], where B is batch size and T is time frames
        :param seq_len: tensor of shape [B], where B is batch size
        :return:
        """
